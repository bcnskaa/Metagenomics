...
melt_df <- function(df_list, new_class_label="class", ignore_na=TRUE) {
	...
	for(label in class_labels) {
		...
		cur_df <- cbind(cur_df, iteration=seq(1, nrow(cur_df)), stringsAsFactors=F)
...
# Usage: 
# main2("group.info", "/disk/rdisk08/yyjia3/16S/All_samples/QIIME_output_forward/bacteria/arare_min_P_1000/alpha_div_collated", "Bacteria")
# main2("group.info", "/disk/rdisk08/yyjia3/16S/All_samples/QIIME_output_forward/archaea/arare_min_P_1000/alpha_div_collated", "Archaea")
main2 <- function(group_info_fn, path, output_prefix=character(0), mean_replicate=FALSE)
{
	alpha_diversities <- c("PD_whole_tree", "chao1", "fisher_alpha", "shannon", "observed_species");
	...
	for(alpha_diversity in alpha_diversities)
	{
		...
		for(label in labels) 
		{
			...
			{
				cat(paste("Warning: ", alpha_diversity, "/", label, " contains N/A values, and is dicarded from further analyses\n", sep=""));
			}
		}
	...
	for(alpha_diversity in alpha_diversities)
	{
		...
		ss <- melt_dfs[melt_dfs$class == alpha_diversity, ];
		
		# Take a mean value from all iterations
		if(flag_mean)
		{	
			require(plyr);
			ss <- ddply(ss, .(variable, class), summarise, value=mean(value));	
		}
	
		
		# Assigning meta info to the ss
		ss <- merge(ss, group_info, by.y="Label", by.x="variable");
		ss <- cbind(ss, id=paste(ss$Place, " ", ss$Treatment, " ", ss$Temperature, "C (", ss$Year, "yr) ", sep=""), stringsAsFactors=F);
		
		# Combined places
		pdf(paste(output_prefix, ".", alpha_diversity,".boxplot.pdf", sep=""), width=4, height=6);
		print(qplot(factor(id), value, data = ss, geom = "boxplot") + geom_jitter(position=position_jitter(w=0.1, h=0.1)) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
						labs(x="", y=paste("", alpha_diversity)) + 
						ggtitle(paste(output_prefix, "\nAlpha Diversity=", alpha_diversity,sep="")));
		dev.off();
	}
}
