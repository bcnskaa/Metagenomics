...
def calculate_mtx_with_coverage(bla_fn, relative_abund=True, path_to_abund_dir="/home/siukinng/MG/scaffolds", path_to_seq_dir="/home/siukinng/MG/scaffolds/CrossValidation/combined/combined_seqs", abund_dir_suffix="_5000"):
    ...
    for b in bla:
        ...
        if s_id in subject_metagenome_abund.keys() and q_id in query_metagenome_abund.keys():
            if relative_abund:
                ...
                s_bla_sum = s_bla_sum + (s_weight * (int(b[3]) - ( int(b[5]) + int(b[4]) )))
                q_bla_sum = q_bla_sum + (q_weight * (int(b[3]) - ( int(b[5]) + int(b[4]) )))
            else:
                s_bla_sum = s_bla_sum + (int(b[3]) - ( int(b[5]) + int(b[4]) ))
                q_bla_sum = q_bla_sum + (int(b[3]) - ( int(b[5]) + int(b[4]) ))
