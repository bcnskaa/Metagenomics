...
# grouping 
#
import_merged_tbl <- function(tbl_merged_fn, normalization=F, selected_sample_ids=character(0), selected_tax_ids=character(0), selected_gene_id=character(0), colorbar_scheme=c("steelblue", "yellow", "darkred"))
{
	...
	ggplot(data=tbl, aes(x=gene_id, y=group_id, fill=count)) + geom_tile() +
	theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust=0.5)) +
	scale_fill_gradient2(name="", low=colorbar_scheme[1], mid=colorbar_scheme[2], high=colorbar_scheme[3], midpoint=15)
