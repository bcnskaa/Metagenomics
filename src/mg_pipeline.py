...
PFAM_HMM = DB_HOME + "/Markers/Pfam/Pfam.hmm"
SPECI_HMM = DB_HOME + "/Markers/specI/specI.hmm"
...
#         run_id = read_1_fn.split("_1.",1)[0]
    ...
    if not run_idba_ud(merged_read_fn, min_contig=2000, maxk=100):
    ...
    run_EMIRGE(read_fns[0], read_fns[1])
   
    # Great, we are moving to binning stages
    maxbin_outdir = run_MaxBin(contig_fn, merged_read_fn, maxbin_outprefix=run_id, min_contig_length=5000)
    ...
    for bin_group_fasta_fn in bin_group_fasta_fns:
        ...
        protein_outfn = prodigal_info["protein_outfn"]
        print_status("Searching " + protein_outfn + " against known Pfam sequences")
        running_HMMER_search(PFAM_HMM, protein_outfn, outdir=MARKER_OUTDIR+"/Pfam")
        print_status("Searching " + protein_outfn + " against marker genes")
        running_HMMER_search(SPECI_HMM, protein_outfn, outdir=MARKER_OUTDIR+"/SpecI")
...
"""
def run_MaxBin(contig_fn, merged_read_fn, maxbin_outdir=BINNING_OUTDIR + "/MaxBin", maxbin_outprefix="contig", thread=16, min_contig_length=1200):
    print_status("Initializing MaxBin for " + maxbin_outprefix)  
    
    cmd = MAXBIN_HOME + "/run_MaxBin.pl -contig " + contig_fn + " -out " + maxbin_outdir + "/" + maxbin_outprefix + " -thread " + str(thread) + " -plotmarker -reads " + merged_read_fn + " -min_contig_length " + str(min_contig_length)
    print_status("command: " + cmd)
...
"""
def run_EMIRGE(read_1_fn, read_2_fn, DIR=MARKER_OUTDIR + "/EMIRGE", out_fa_fn=None, ssu_fn=EMIRGE_16S_DB, ssu_bowtie_fn=EMIRGE_16S_DB, platform_illumina=True, num_threads=16):
    ...
    cmd = EMIRGE_HOME + "/bin/emirge.py " + DIR + " -1 " + read_1_fn + " -2 " + read_2_fn + " -f " + ssu_fn + " -b " + ssu_bowtie_fn + " -a " + str(num_threads) + " -l " + str(read_summary["max_read_length"]) + " -i " + str(int(math.ceil(read_summary["insert_size_mean"]))) + " -s " + str(int(math.ceil(read_summary["insert_size_sd"])))
...
"""
def map_scaffold_ids2_bin_id(scaffold_ids, maxbin_dir, bin_fa_fn_ext=".fna"):
