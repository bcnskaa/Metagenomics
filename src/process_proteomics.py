...
2. Convert the wiff file into mgf format by using AB_SCIEX_MS_Converter.exe
   ...
   # /doubleprecision does not work 
   > AB_SCIEX_MS_Converter.exe WIFF IN_FILE -proteinpilot MGF OUT_FILE /nocompression /doubleprecision /index
   # /doubleprecision does not work 
...
process_proteomics.process_proteomics("interact-GZ-Xyl.ipro.prot.xls", fa_fn="GZ-Xyl+DECOY.faa", map_fn="/home/siukinng/MG/m8/proteins/NR/renamed/all_samples+nr.renamed.m8.map_gi", map_to_nr=True, summary_follows_lst_format=True)
process_proteomics.process_proteomics("interact-SWH-Xyl.ipro.prot.xls", fa_fn="SWH-Xyl+DECOY.faa", map_fn="/home/siukinng/MG/m8/proteins/NR/renamed/all_samples+nr.renamed.m8.map_gi", map_to_nr=True, summary_follows_lst_format=True)

"""
# lst_fn: *.ipro.prot.xls generated by TPP
# fa_fn: File contains sequences of query sequences
# map_fn: gi
def process_proteomics(lst_fn, fa_fn=None, map_fn=None, lst_ofn=None, map_to_nr=False, summary_follows_lst_format=False, gi2ctx_map_fn="/home/siukinng/db/Markers/ncbi_nr/gi2ctx.map"):
    ...
    if map_fn is not None:
        ...
        # m8 file containing PIDs to GIs and gi_ctx
        qid2gi_map = [m.split("\t") for m in qid2gi_map]

#         for i, g in enumerate(qid2gi_map):
#             if g[1].startswith("gi"):
#                 qid2gi_map[i][1] = g[1].split("|")[1]
        # 
        print("qid2gi_map: " + "##".join(qid2gi_map[0]))
...
#         
        ...
        #map_dat = map_id_to_gi_ctx(pid2gi_map)
...
"""
Reformat the list file from BLAST tabular format into the format follows the list file
"""
...
#     
    ...
    for i, l in enumerate(lst_dat):
        ...
        print("Protein_IDs=" + str(len(protein_ids)))
...
"""
Given mapping data (map_dat) and list file, this function will link rows in list file to mapping data.
map_dat: 
lst_fn: the .prot.xls file generated by TPP.

"""
def export_summary(map_dat, lst_fn, summary_ofn=None, reformated_summary_ofn=None, gi2ctx_map=None, delim=";", list_delim=","):
    ...
    # Mapping data
    map_dat_lst = {m[0] : m[1] for m in map_dat}
...
# #             gi = gi + ":" + gi2ctx_map[gi]
# #         except:
# #             gi = gi + ":Unknown_Function"
#             
#         try:
#             
#             #map_dat_lst[gi].append(m[1])
#             map_dat_lst[m[0]].append(gi)
#         except:
#             map_dat_lst[m[0]] = [gi]
    ...
    # Pick protein ids from the list file
    ids = pick_protein(lst_fn, p_val_threshold=p_value_threshold)
    print("Number of Protein IDs: " + str(len(ids)))
    print("IDs[0]=" + ids[0])
...
def import_gi2ctx_map(gi2ctx_map_fn="/home/siukinng/db/Markers/ncbi_nr/gi2ctx.map"):
#def import_gi2ctx_map(gi_fn="/home/siukinng/db/BacteriaDB/all_faa.gi.lst"):
    print("Importing gi2ctx_map from " + gi2ctx_map_fn)
    with open(gi2ctx_map_fn) as IN:
